#include "ncurses.h"
#include <arpa/inet.h>
#include <stdlib.h>
#include "string.h"
#include "pthread.h"

#define MAX_CLIENTS 10
#define PORT 40000


#define CLIENT_BUFFER 1024

int active_connections = 0;

struct player {
    int id;
    int pos_y, pos_x;
    char character;
    int color;
};

typedef struct player player_s;

struct client {
    int id;
    char is_use;
    int fd_socket;
    struct sockaddr_in address;
    void *data;
};

typedef struct client client_s;

client_s clients[MAX_CLIENTS];

// 0 1 2 3 4 5 6 7


void init_client_s_struct(int max_clients, client_s clients[max_clients])
{
    for (int i = 0; i < max_clients; i++) {
        //clients[i].is_use = 0;
        //clients[i].fd_socket = 0;
        //memset(&clients[i].socket, 0, sizeof(struct sockaddr_in));
        memset(&clients[i], 0, sizeof(client_s));
        clients[i].id = i;
    }
}

player_s *init_player_s_struct(int id, int pos_y, int pos_x, char character, int color)
{
    player_s *player = (player_s *) malloc(sizeof(player_s));
    if (!player) {
        perror("error init_player_s_struct");
        exit(84);
    }
    player->id = id;
    player->pos_y = pos_y;
    player->pos_x = pos_x;
    player->character = character;
    player->color = color;

    return player;
}


client_s *find_free_slot(int max_clients, client_s clients[max_clients])
{
    for (int i = 0; i < max_clients; ++i) {
        if (clients[i].is_use == 0) {
            return &clients[i];
        }
    }
    return NULL;
}


void *handle_client(void *data)
{
    client_s *client = (client_s *) data;
    char buffer[CLIENT_BUFFER] = {0};

    while(1) {
        ssize_t n = recv(client->fd_socket, buffer, CLIENT_BUFFER, MSG_DONTWAIT); // -1 0
        if (n == 0) {
            int temp_id = client->id;
            free((client_s *)client->data);
            memset(client, 0, sizeof(client_s));
            client->id = temp_id;
            break;
        }
        buffer[n] = 0;
    }
    return NULL;
}

int handle_server()
{
    //initscr();
    //curs_set(FALSE);
    //noecho();

    int server_fd, new_socket;
    struct sockaddr_in address;
    int opt = 1;

    int addrlen = sizeof(address);
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        perror("socket failed");
        exit(EXIT_FAILURE);
    }

    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }

    address.sin_family = AF_INET;
    address.sin_addr.s_addr = INADDR_ANY;
    address.sin_port = htons(PORT);

    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }

    if (listen(server_fd, MAX_CLIENTS) < 0) {
        perror("listen");
        exit(EXIT_FAILURE);
    }


    while (1) {
        //printf("\nWaiting for a connection...\n");
        //mvprintw(0, 0, "Waiting for a connection...\n");
        //refresh();
        int free_socket = find_free_slot(MAX_CLIENTS, clients)->id;
        client_s *current_client = &clients[free_socket];

        if ((current_client->fd_socket = accept(server_fd, (struct sockaddr_in *) &current_client->address, (socklen_t*)&addrlen)) < 0) {
            perror("accept");
            exit(EXIT_FAILURE);
        }

        current_client->is_use = 1;
        current_client->data = init_player_s_struct(free_socket, 0, 0, '#', COLOR_BLACK);

        //FIRST CONNECTION
        send(current_client->fd_socket, (player_s *) current_client->data, sizeof(player_s), 0);

        pthread_t thread;

        if (pthread_create(&thread, NULL, handle_client, &current_client) != 0) {
            perror("pthread_create");
        } else {
            pthread_detach(thread);
        }


        //erase();
        for (int i = 0, current = 0; i < MAX_CLIENTS; i++) {
            if (clients[i].is_use) {
                char *client_ip = inet_ntoa(clients[i].address.sin_addr);
                //printf("New connection from %s\n", client_ip);
                //mvprintw(current + 1, 0, "Client n%d :%s", clients[i].id, client_ip);
                printf("Client n%d :%s\n", clients[i].id, client_ip);
                current++;
            }
        }
        //refresh();
    }
}



int main(int argc, char *argv[])
{
    init_client_s_struct(MAX_CLIENTS, clients);

    //client_s *my_free_slot = find_free_slot(MAX_CLIENTS, clients);

    handle_server();
    return 0;
}